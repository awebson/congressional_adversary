...
class Decomposer(nn.Module):
    ...
    def __init__(
        ...
        # temp_path = '../results/analysis/L4 0c/epoch50.pt'
        ...
        # Dennotation Loss: Bill Topic or Title Prediction
        # self.deno_decoder = config.deno_architecture


        self.deno_trsf = nn.Transformer(
            d_model=300,
            nhead=4,
            ...
            dropout=0.1)
        self.deno_lin = nn.Linear(300, 41)

        self.cono_trsf = nn.Transformer(
            d_model=300,
            nhead=4,
            num_encoder_layers=1,
            num_decoder_layers=1,
    ...
    def forward(
        ...
        words: R3Tensor = self.embedding(seq_word_ids)
        init_target_seq = torch.zeros(1, seq_word_ids.shape[1], 300, device=self.device)

        deno_repr = self.deno_trsf(words, init_target_seq)
        deno_logits = self.deno_lin(deno_repr.squeeze())
        deno_loss = nn.functional.cross_entropy(deno_logits, deno_labels)

        cono_repr = self.cono_trsf(words, init_target_seq)
        cono_logits = self.cono_lin(cono_repr.squeeze())
    ...
    def predict(self, seq_word_ids: Vector) -> Vector:
        ...
        with torch.no_grad():
            ...
            deno_conf = nn.functional.softmax(deno_logits, dim=1)
            cono_conf = nn.functional.softmax(cono_logits, dim=1)
        self.train()
...
class LabeledSentences(Dataset):
    ...
    def collate(
        ...
        return (
            rnn.pad_sequence(seq_word_ids, batch_first=False),
            deno_labels,
            cono_labels)
...
class DecomposerExperiment(Experiment):
    ...
    def __init__(self, config: 'DecomposerConfig'):
        ...
        self.deno_params = list(self.model.deno_trsf.parameters()) + list(self.model.deno_lin.parameters())
        self.cono_params = list(self.model.cono_trsf.parameters()) + list(self.model.cono_lin.parameters())
        self.f_deno_optimizer = config.optimizer(self.deno_params, lr=config.learning_rate)
        self.f_cono_optimizer = config.optimizer(self.cono_params, lr=config.learning_rate)
        ...
        self.decomposer_optimizer = config.optimizer(
            self.model.embedding.parameters(),  # NOTE trainable embedding
            lr=config.learning_rate)
    ...
    def _train(
        ...
        # L_master, l_f_deno, l_f_cono = self.model(
        ...
        # self.model.zero_grad()
        # L_master.backward()  # NOTE deleted retain_graph=True
        ...
        L_master.backward(retain_graph=True)  # NOTE deleted retain_graph=True
        nn.utils.clip_grad_norm_(self.model.embedding.parameters(), grad_clip)
        # nn.utils.clip_grad_norm_(self.model.decomposer_g.encoder.parameters(), grad_clip)
        self.decomposer_optimizer.step()
...
class DecomposerConfig():
    ...
    def __post_init__(self) -> None:
        ...
        # # Unprincipled Per Word Prediction
        # transformer_layer = nn.TransformerEncoderLayer(
        #     d_model=300,
        #     nhead=1,
        #     dim_feedforward=1024,  # default 2048
        #     dropout=0.1)
        # attention = nn.TransformerEncoder(transformer_layer, num_layers=1)
        ...
        #         # nn.SELU()
        #     )
        #     # self.deno_architecture = attention, nn.Linear(1024, 41)
